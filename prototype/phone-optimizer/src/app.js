...
const I18N = {
  fa: {
    ...
    "apps.ob_native": (n) => `${faN(n)} برنامهٔ واقعی انتخاب شد`,
    ...
    "boost.result.t2": "بستن شروع شد",
    "boost.result.s": (min, sc) => `≈ ${faN(min)} دقیقه عمر باتری بیشتر • امتیاز سلامت: ${faN(sc)}`,
    "boost.result.native": "سرویس دسترس‌پذیری بهینه داخل تنظیمات روی «توقف اجباری» هر برنامه می‌زند — صفحه را دنبال کن",
    /* misc */
    ...
    "toast.usageperm": "مجوز «دسترسی به آمار استفاده» را بده تا برنامه‌های واقعی لود شوند",
    "toast.accperm": "اول سرویس دسترس‌پذیری بهینه را فعال کن",
  ...
  en: {
    ...
    "apps.ob_native": (n) => `${n} real apps selected`,
    ...
    "boost.done": "Got it!",
    "boost.result.t2": "Closing started",
    "boost.result.s": (min, sc) => `≈ ${min} min extra battery • health score: ${sc}`,
    "boost.result.native": "Behine's accessibility service is tapping Force stop in Settings — watch it work",
    ...
    "toast.usageperm": "Grant Usage access to load real apps",
    "toast.accperm": "Enable Behine's accessibility service first",
...
function renderApps() {
  ...
  const sel = closable();
  const nativeSel = apps.filter(a => a.native && a.selected && !a.closed).length;
  $("obText").innerHTML = nativeSel ? t("apps.ob_native", nativeSel)
    : sel.length ? t("apps.ob_some", sel.length, sel.reduce((s, a) => s + a.ram, 0)) : t("apps.ob_none");
  $("optimizeBtn").style.display = (sel.length || nativeSel) ? "" : "none";
  ...
  [...apps].sort((a, b) => (a.closed ? 1 : 0) - (b.closed ? 1 : 0) || b.ram - a.ram).forEach(a => {
    ...
    const badge = a.closed ? `<span class="ai-badge ab-closed">${t("badge.closed")}</span>`
      : a.native ? `<span class="ai-badge ab-ai">${t("badge.native")}</span>`
    ...
    row.innerHTML = `
      ...
      <div class="app-meta">
        ...
        <small>${a.native ? t("app.installed") : `${t("app.mb", N(a.ram))} · ${t("app.drain", N(a.drain, 1))} · ${t("app.idle", N(a.idle))}`}</small>
      </div>
      ${badge}${sw}`;
    list.appendChild(row);
  });
...
async function runBoost() {
  if (boosting) return;
  if (window.BehineNative) return runNativeBoost();
...
/* ── Native boost (APK · real force-close via accessibility) ── */
async function runNativeBoost() {
  const nat = window.BehineNative;
  const targets = apps.filter(a => a.native && a.selected && !a.closed);
  if (!targets.length) { toast(t("toast.nothing")); return; }
  if (!nat.isAccEnabled()) { toast(t("toast.accperm")); nat.openAccSettings(); return; }
  boosting = true;
  flowStart("rd-boost", t("boost.scan"));
  await sleep(900); flowPct(25);
  $("boTitle").textContent = t("boost.close");
  targets.forEach(a => flowLine(a.ic, a.en, "…"));
  try { nat.forceClose(JSON.stringify(targets.map(x => x.pkg))); } catch (e) {}
  await sleep(1200 + targets.length * 2600);
  targets.forEach(a => { a.closed = true; a.selected = false; });
  flowPct(100);
  flowFinish("🤖", t("boost.result.t2"), "", t("boost.result.native"), t("boost.done"),
    () => { boosting = false; renderAll(); toast(t("toast.boosted")); });
}
...
/* ── Native bridge (running inside Behine APK) ── */
(function initNative() {
  const nat = window.BehineNative;
  if (!nat) return;
  ...
  try {
    const syncBatt = () => {
      const b = J(nat.getBattery());
      if (b) {
        if (typeof b.level === "number") state.battery.level = b.level;
        state.battery.charging = !!b.charging;
        if (typeof b.temp === "number") state.temp = b.temp;
        renderDashboard(); renderBattery();
      }
    };
    ...
    if (Array.isArray(top) && top.length) {
      apps = top.slice(0, 12).map((a, i) => ({
        id: "n" + i, pkg: a.pkg, en: a.label, fa: a.label, ic: "📦",
        ram: 0, drain: 0, idle: 0, freq: 0.5, native: true, closed: false, selected: false,
      }));
      renderApps(); renderDashboard();
    } else {
      toast(t("toast.usageperm"));
    }
  } catch (e) {}
})();
